...
{
    public async Task<Membership?> GetByIdWithIncludesAsync(int id, CancellationToken ct = default)
    ...
    public async Task<bool> CancelMembershipByIdAsync(int membershipId, CancellationToken ct = default)
    {
        var membership = await dbContext.Memberships
            .Include(m => m.Member)
            .FirstOrDefaultAsync(m => m.Id == membershipId && !m.IsDeleted, ct);
    ...
    {
        var membership = await dbContext.Memberships
            .FirstOrDefaultAsync(m => m.MemberId == memberId && m.EndDate > DateOnly.FromDateTime(DateTime.Now) && !m.IsDeleted, ct);
